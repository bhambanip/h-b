...
$fileToWrite = fopen(asiAmzFile, "w");
fputcsv($fileToWrite, $amzFirstRow, "\t");

$result = mysqli_query($conn, "SELECT * FROM asiItemList") or die("database error:" . mysqli_error($connString));
...
foreach ($result as $row) {
    if ($row['Sku'] && $row['Regulation'] && $row['Status']) {
        ...
        if (strpos( $row['LongDesc'], 'Tower') !== false) {
            $price = $price + 30;
        }
        if ($row['AmzFees'] == 0) {
            $priceQuery = mysqli_query($conn, "SELECT * FROM asiCatList where SubCategoryNo ='" . $row['SubCategoryNo'] . "'
        and CategoryNo ='" . $row['CategoryNo'] . "'");
