...
$fileToWrite = fopen(ingramAmzFile, "w");
fputcsv($fileToWrite, $amzFirstRow, "\t");

$result = mysqli_query($conn, "SELECT * FROM ingramItemList") or die("database error:" . mysqli_error($connString));
...
foreach ($result as $row) {
    if ($row['Sku'] && $row['Regulation'] && $row['Status']) {
        ...
        if ($row['SubCategoryNo'] == '0303LCD') {
            $price = $price + 12;
        }

        if ((($row['Height'] > 30.00) || ($row['Length'] > 30.00)) && ($row['Sku'] != '5073GC')) {
            $price += amzOverSize;
        }
