...
foreach ($itemTables as $table) {
    $query = mysqli_query($conn, "SELECT * FROM " . $table . " where (Quantity != 0) and (Status != 0)  and (AmzStatus != 0) and (AmzLowestPrice = 1) 
    and (Date(AmzLowestPriceUpdate) != Date(now())) Order by Id");
